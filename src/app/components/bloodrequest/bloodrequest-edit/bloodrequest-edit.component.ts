...
@Component({
  selector: 'app-bloodrequest-edit',
  templateUrl: './bloodrequest-edit.component.html',
  styleUrls: ['./bloodrequest-edit.component.css']
})
export class BloodrequestEditComponent implements OnInit {
  ...
  update(): void {
    ...
    this.requestSvc.updateRequest(this.bloodRequest)
      .subscribe(r => {
  ...
  ngOnInit(): void {
    ...
    this.requestSvc.getRequestById(id)
      .subscribe(x => {
        this.bloodRequest = x;
        this.requestForm.patchValue(this.bloodRequest);
      }, err => {
        this.notifySvc.fail("Failed to load request data", "DISMISS");
      })
